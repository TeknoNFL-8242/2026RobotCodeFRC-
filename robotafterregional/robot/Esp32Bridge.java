...
public class Esp32Bridge {
  ...
  public Esp32Bridge(Integer csDioChannel) {
    spi = new SPI(RobotConfig.Imu.SPI_PORT);
    spi.setClockRate(RobotConfig.Imu.SPI_CLOCK_HZ); // 1 MHz öneri (ESP tarafı max 3MHz)
  ...
  public float[] readEuler() {
    byte[] tx1 = new byte[RobotConfig.Imu.TRANSFER_SIZE];
    byte[] rx1 = new byte[RobotConfig.Imu.TRANSFER_SIZE];
    tx1[0] = (byte) RobotConfig.Imu.CMD_REQUEST_EULER;
    ...
    int rc1 = spi.transaction(tx1, rx1, RobotConfig.Imu.TRANSFER_SIZE);
    if (csManual != null) csManual.set(true); // CS deassert
    if (rc1 != RobotConfig.Imu.TRANSFER_SIZE) {
      // Başarısız veya eksik transfer
      return null;
    }

    // Kısa bekleme; ESP32 bu süre içinde cevabı hazırlar
    Timer.delay(RobotConfig.Imu.PREP_DELAY_SECONDS);

    // Transaction 2: karşı taraftan hazır veriyi oku (MASTER clocks out slave TX)
    byte[] tx2 = new byte[RobotConfig.Imu.TRANSFER_SIZE]; // dummy (sıfırlar)
    byte[] rx2 = new byte[RobotConfig.Imu.TRANSFER_SIZE];
    if (csManual != null) csManual.set(false);
    int rc2 = spi.transaction(tx2, rx2, RobotConfig.Imu.TRANSFER_SIZE);
    if (csManual != null) csManual.set(true);
    if (rc2 != RobotConfig.Imu.TRANSFER_SIZE) {
      return null;
    }
